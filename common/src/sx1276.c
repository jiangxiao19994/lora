 ...
 */
DioIrqHandler *DioIrq[] = { SX1276OnDio0Irq, NULL,
                            NULL, NULL,
                            NULL, NULL };
...
void TxTimerInit(unsigned int time)
 {
       struct itimerval value;
       
       value.it_value.tv_sec = 0;
       value.it_value.tv_usec = 1000 * time;
...
void SX1276SetChannel( uint32_t freq )
{
	...
	SX1276Write(REG_LR_FRFMSB, ( channel >> 16 ) & 0xFF);
	SX1276Write(REG_LR_FRFMID, ( channel >> 8 ) & 0xFF);
	SX1276Write(REG_LR_FRFLSB, ( channel >> 0 ) & 0xFF);
}
...
{
    // Set RESET pin to 0
    set_gpio_ouput(gpio_fd,reset_group[index],reset_num[index]);
    set_gpio_value(gpio_fd,reset_group[index],reset_num[index], 0);
    // Wait 1 ms
    DelayMs( 1 );

    set_gpio_input(gpio_fd,reset_group[index],reset_num[index]);
    // Wait 6 ms
    DelayMs( 6 );
}
